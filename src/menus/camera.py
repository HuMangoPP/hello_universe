...
class Camera():
    def __init__(self, follow: np.ndarray, res: tuple):
    ...
    def transform_to_screen(self, pos: np.ndarray):
        ...
        return self.dir_transform(np.array(pos)-self.pos)+np.array([self.width//2, self.height//2])
    ...
    def screen_to_world(self, x: float, y: float):
        pos = np.array([x,y,0])-np.array([self.width//2, self.height//2,0])
