...
#         self.attached_segments = np.array([])
#         self.leg_types = []

#         # rendering and physics
#         self.feet_pos:list[np.ndarray] = []
#         self.step_pos:list[np.ndarray] = []
#         self.step_bend:float = step_bend
#         self.feet_size:int = size
    
#     def build_legs(self, body_seg_pos: np.ndarray):
#         self.feet_pos.append(
#             body_seg_pos + np.array([0,self.leg_length/2,0])
#         )
#         self.step_pos.append(
#             body_seg_pos + self.leg_length*np.array([cos(self.step_bend), sin(self.step_bend), 0])
#         )
#         self.feet_pos.append(
#             body_seg_pos - np.array([0, self.leg_length/2])
#         )
#         self.step_pos.append(
#             body_seg_pos + self.leg_length*np.array([cos(-self.step_bend), sin(-self.step_bend), 0])
#         )
...
#                 screen_pos.append(camera.transform_to_screen(self.feet_pos[2*i]))
#                 screen_pos.append(camera.transform_to_screen(skeleton[self.attached_segments[i]]))
#                 pg.draw.circle(screen, MODEL_COLORS['foot'], screen_pos[0], self.feet_size)
#                 pg.draw.line(screen, MODEL_COLORS['leg'], screen_pos[0],
#                                             screen_pos[1])
                
#                 screen_pos = []
#                 screen_pos.append(camera.transform_to_screen(self.feet_pos[2*i+1]))
#                 screen_pos.append(camera.transform_to_screen(skeleton[self.attached_segments[i]]))
#                 pg.draw.circle(screen, MODEL_COLORS['foot'], screen_pos[0], self.feet_size)
#                 pg.draw.line(screen, MODEL_COLORS['leg'], screen_pos[0],
#                                             screen_pos[1])
...
class Legs:
    def __init__(self, num_pair_legs: int, leg_length: int, step_bend=math.pi/6, feet_size=5):
        self.num_pair_legs = num_pair_legs
        self.leg_length = leg_length
        self.skeleton_segments_with_legs = np.array([])

        self.feet_pos:np.ndarray = np.array([])
        self.step_pos:np.ndarray = np.array([])
        self.step_bend = step_bend
        self.feet_size = feet_size
    
    def build_legs(self, skeleton: np.ndarray, flat_angles: np.ndarray):
        attached_skeleton = skeleton[self.skeleton_segments_with_legs]
        self.step_pos = np.array([[
            segment + self.leg_length * np.array([math.cos(flat_angle+self.step_bend), math.sin(flat_angle+self.step_bend), 0]),
            segment + self.leg_length * np.array([math.cos(flat_angle-self.step_bend), math.sin(flat_angle-self.step_bend), 0])]
            for segment, flat_angle in zip(attached_skeleton, flat_angles)])
        self.feet_pos = self.step_pos.copy()

    def update(self, skeleton: np.ndarray, flat_angles: np.ndarray):
        attached_skeleton = skeleton[self.skeleton_segments_with_legs]
        self.step_pos = np.array([[
            segment + self.leg_length * np.array([math.cos(flat_angle+self.step_bend), math.sin(flat_angle+self.step_bend), 0]),
            segment + self.leg_length * np.array([math.cos(flat_angle-self.step_bend), math.sin(flat_angle-self.step_bend), 0])]
        ...
        new_feet_pos = []
        for segment, foot_pair, step_pair in zip(attached_skeleton, self.feet_pos, self.step_pos):
            dist = np.linalg.norm(foot_pair[0] - segment)
            if dist > self.leg_length:
                first_foot = step_pair[0]
            else:
                first_foot = foot_pair[0]
            dist = np.linalg.norm(foot_pair[1] - segment)
            if dist > self.leg_length:
                second_foot = step_pair[1]
            else:
                second_foot = foot_pair[1]
            new_feet_pos.append([first_foot, second_foot])
        self.feet_pos = np.array(new_feet_pos)
    
    def render(self, skeleton: np.ndarray, flat_angles: np.ndarray, display: pg.Surface, camera):
        attached_skeleton = skeleton[self.skeleton_segments_with_legs]
        for segment, flat_angle, foot_pair in zip(attached_skeleton, flat_angles, self.feet_pos):
            sx,sy = camera.transform_to_screen(segment)
            
            jx,jy = camera.transform_to_screen(self.get_joint_pos(segment, flat_angle, foot_pair[0]))
            x,y = camera.transform_to_screen(foot_pair[0])
            pg.draw.lines(display, MODEL_COLORS['leg'], False, [(sx,sy),(jx,jy),(x,y)])
            pg.draw.circle(display, MODEL_COLORS['foot'], (x,y), self.feet_size)

            jx,jy = camera.transform_to_screen(self.get_joint_pos(segment, flat_angle, foot_pair[1], neg=-1))
            x,y = camera.transform_to_screen(foot_pair[1])
            pg.draw.lines(display, MODEL_COLORS['leg'], False, [(sx,sy),(jx,jy),(x,y)])
            pg.draw.circle(display, MODEL_COLORS['foot'], (x,y), self.feet_size)
    
    def get_joint_pos(self, segment: np.ndarray, angle: float, foot_pos: np.ndarray, neg=1):
        forward = np.array([math.cos(angle), math.sin(angle), 0])
        leg = foot_pos - segment
        joint_dir = neg * np.cross(forward, leg)
        joint_dir = joint_dir / np.linalg.norm(joint_dir)
        mag = math.sqrt(abs(self.leg_length**2/4 - np.linalg.norm(leg)**2/2))
        joint_dir = mag * joint_dir
        return segment + leg/2 + joint_dir
